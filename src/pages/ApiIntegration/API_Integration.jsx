...
function ServerRackVisual() {
  return (
    <div className="rounded-2xl overflow-hidden shadow-2xl aspect-[4/3] bg-gradient-to-br from-gray-900 via-slate-800 to-gray-950 relative flex items-center justify-center">
      <div className="absolute inset-0 flex flex-col justify-center gap-1.5 px-6 py-8">
        {[...Array(9)].map((_, i) => (
          <div
            key={i}
            className="flex items-center gap-2 bg-gradient-to-r from-gray-800 to-gray-750 border border-gray-700/60 rounded px-3 py-1.5"
          >
            <div
...
const serviceItems = [
  {
    ...
    desc: "Tailor-made RESTful and GraphQL APIs built to handle your specific data and logic needs. We ensure your data is accessible, performant, and perfectly structured for your frontend needs.",
    tags: [
  ...
  },
  {
    ...
    desc: "Seamlessly connect your software with global platforms. We handle the complexities of authentication, rate limiting, and data mapping so you don't have to.",
    extra: (
      <div className="flex flex-wrap gap-2 mt-2">
        {["STRIPE", "SALESFORCE", "AWS", "TWILIO"].map((p) => (
          <span
            key={p}
            className="text-xs font-semibold bg-gray-100 text-gray-600 px-3 py-1 rounded-full border border-gray-200"
          >
            {p}
          </span>
        ))}
  ...
  },
  {
    ...
    desc: "Transition from monoliths to scalable microservices for better performance and agility. Decouple your services to allow independent scaling and deployment.",
    extra: (
      <AnimatedBar
  ...
  },
  {
    ...
    title: "API Security",
    desc: "Enterprise-grade security featuring OAuth2, JWT, and advanced encryption protocols. We protect your data at rest and in transit against modern threats.",
    extra: (
      <div className="flex gap-3 flex-wrap mt-2">
        {["OAuth 2.0", "SSL/TLS 1.3"].map((s) => (
          <div key={s} className="flex items-center gap-1.5">
            <FiShield className="w-3.5 h-3.5 text-blue-500" />
            <span className="text-gray-500 text-xs font-medium">{s}</span>
          </div>
        ))}
      </div>
    ),
